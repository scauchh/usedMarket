...
// 刷新数据
const refresh = async () => {
  let result = await getTradeFromMeService(pageNum.value, pageSize.value, searchData.value)
...
const confirm = async (row) => {
  ElMessageBox.confirm("请确保您已完成交易", "交易提示", {
      confirmButtonClass: "确定",
      cancelButtonClass: "取消",
    }).then(async() => {
      await updateTrade(row.tradeID, 3, '')
...
const cancel = async (row) => {
  ElMessageBox.confirm("您确定要取消该交易吗?", "交易提示", {
      confirmButtonClass: "确定",
      cancelButtonClass: "取消",
    }).then(async() => {
      await updateTrade(row.tradeID, 4, '买家取消交易')
  ...
  <el-card class="page-container">
    ...
    <el-table :data="tradeInfo" style="width: 100%">
      ...
      <el-table-column label="" width="300">
        <template #default="{ row }">
          <el-text v-if="row.state==1">请等待卖家确认</el-text>
          <el-text v-else-if="row.state==2" inline>
            <el-button type="success" plain @click="confirm(row)">确认收货</el-button>
            <el-button type="danger" plain @click="cancel(row)">取消交易</el-button>
          </el-text>
    ...
    <!-- 卖家信息弹窗 -->
    <el-dialog v-model="visibleUser" title="卖家信息" width="30%">
